...
char* convertString(TagLib::String tstr);
...
{	
	...
	*/
	...
	post("vb.metatag, based on TagLib 1.11");
	return 0;
}
...
void metatag_set(t_metatag *x, t_symbol *s, long ac, t_atom *av)
{
	...
	if (av && atom_gettype(av) == A_SYM) {
		...
		object_error((t_object *)x, "set error: first arg must be a tag symbol");
		return;
	}
...
void metatag_get_all(t_metatag *x) {
	...
	if(!f.isNull()) {
		...
		for(i = tags.begin(); i != tags.end(); ++i) {
			key = gensym(convertString(i->first));
...
void metatag_writeToFile(t_metatag *x) {
	...
	if(x->fileref) {
		f = *x->fileref;
		if(!f.isNull()) {
			object_post((t_object *)x, "saving to file");
			f.file()->save();
		}
	}
	else {
...
void printProperties(t_metatag *x)
{
	...
	if(!f.isNull() && f.tag()) {
		...
		tstr = tag->artist();
		post("artist %s", convertString(tstr));
		
		tstr = tag->album();
		post("album %s", convertString(tstr));
		
		tstr = tag->comment();
		post("comment %s", convertString(tstr));
		
		tstr = tag->genre();
		post("genre %s", convertString(tstr));
